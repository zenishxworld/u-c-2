...
// ─── Production note ─────────────────────────────────────────────────────────
// All console.* noise is suppressed at the entry-point (main.tsx) in production.
// Do NOT add ngrok-skip-browser-warning here; that header is dev-only and will
// be ignored (or cause issues) in production anyway.
// ─────────────────────────────────────────────────────────────────────────────
...
const apiRequest = async (endpoint: string, options: RequestInit = {}): Promise<unknown> => {
  ...
  try {
    ...
    if (data && (data as Record<string, unknown>).success && (data as Record<string, unknown>).data !== undefined) {
      return (data as Record<string, unknown>).data;
    }

    return data;
  } catch (error) {
    // Return empty arrays for list-style endpoints so the UI degrades gracefully
...
export const universityAPI = {
  ...
  getDynamicFilters: async (filterBy: string | null = null) => {
